...
TEST(ShapeFunctions, FourNodeSumToOne) {
  auto shape = shape_function(0.0, 0.0, 4);
  type_real sum = 0;
  for (auto v : shape)
    sum += v;
...
TEST(ShapeFunctions, NineNodeSumToOne) {
  auto shape = shape_function(0.5, -0.5, 9);
  type_real sum = 0;
  for (auto v : shape)
    sum += v;
  EXPECT_NEAR(sum, 1.0, 1e-6);
...
TEST(ShapeFunctions, FourNodeDerivativesSumToZero) {
  auto ders = shape_function_derivatives(0.0, 0.0, 4);
...
TEST(ShapeFunctions, NineNodeDerivativesSumToZero) {
  auto ders = shape_function_derivatives(0.1, -0.2, 9);
  ASSERT_EQ(ders.size(), 2); // ndim = 2
...
TEST(ShapeFunctions, InvalidNodeCountThrows) {
  EXPECT_THROW(shape_function(0.0, 0.0, 5), std::invalid_argument);
  EXPECT_THROW(shape_function_derivatives(0.0, 0.0, 7), std::invalid_argument);
}

TEST(ShapeFunctions, KnownValues) {
  // Test at corners for 4-node
  auto shape4 = shape_function(-1.0, -1.0, 4);
  EXPECT_NEAR(shape4[0], 1.0, 1e-6);
  for (int i = 1; i < 4; ++i)
    EXPECT_NEAR(shape4[i], 0.0, 1e-6);

  shape4 = shape_function(1.0, -1.0, 4);
  ...
  auto shape9 = shape_function(-1.0, -1.0, 9);
  EXPECT_NEAR(shape9[0], 1.0, 1e-6);
  for (int i = 1; i < 9; ++i)
    EXPECT_NEAR(shape9[i], 0.0, 1e-6);

  shape9 = shape_function(1.0, -1.0, 9);
  ...
  shape9 = shape_function(0.0, 0.0, 9);
  EXPECT_NEAR(shape9[8], 1.0, 1e-6);
  for (int i = 0; i < 9; ++i)
    if (i != 8)
      EXPECT_NEAR(shape9[i], 0.0, 1e-6);

  shape9 = shape_function(0.0, -1.0, 9);
  EXPECT_NEAR(shape9[4], 1.0, 1e-6);
  for (int i = 0; i < 9; ++i)
    if (i != 4)
      EXPECT_NEAR(shape9[i], 0.0, 1e-6);

  shape9 = shape_function(1.0, 0.0, 9);
